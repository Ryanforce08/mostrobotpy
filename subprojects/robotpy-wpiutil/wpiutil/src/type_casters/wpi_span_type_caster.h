...
template <typename Type, size_t Extent> struct type_caster<std::span<Type, Extent>> {
    ...
    NB_TYPE_CASTER(span_type, io_name(NB_TYPING_SEQUENCE, NB_TYPING_TUPLE) + const_name("[") + span_name_maker<Extent>::make(Caster::Name) + const_name("]"));
...
template <typename Type> struct type_caster<std::span<Type, std::dynamic_extent>> {
    ...
    NB_TYPE_CASTER(span_type, io_name(NB_TYPING_SEQUENCE, NB_TYPING_LIST) + const_name("[") + Caster::Name + const_name("]"));
